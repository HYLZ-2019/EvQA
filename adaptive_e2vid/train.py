...
def create_dataloader(dataset, configs, batch_size, local_rank):
	...
	dataset_collate_fn = configs.get("dataset_collate_fn", None)
	if dataset_collate_fn is not None:
		collate_fn = get_obj_from_str(dataset_collate_fn)
	else:
		collate_fn = None
	
	dataloader = DataLoader(dataset, batch_size=batch_size, sampler=sampler, num_workers=num_workers, persistent_workers=persistent_workers, pin_memory=pin_memory, drop_last=True, collate_fn=collate_fn)
	return dataloader
...
def train(model_interface, dataloader, optimizer, device, writer, epoch, local_rank, make_preview=False):
	...
	for batch_idx, batch in enumerate(dataloader):
		...
		clip_grad_norm_(model_interface.e2vid_model.parameters(), max_norm=1.0)

		# Log gradient and parameter stats
		if (local_rank is None or local_rank == 0) and batch_idx % 10 == 0:
			global_step = epoch * len(dataloader) + batch_idx
			
			all_grads = []
			all_params = []
			for p in model_interface.e2vid_model.parameters():
				if p.grad is not None:
					all_grads.append(p.grad.data.abs().view(-1))
				all_params.append(p.data.abs().view(-1))
			
			if all_grads:
				all_grads_tensor = torch.cat(all_grads)
				writer.add_scalar('stats/grad_avg_abs', all_grads_tensor.mean().item(), global_step)
				writer.add_scalar('stats/grad_max_abs', all_grads_tensor.max().item(), global_step)
				
				# Also keep the L2 norm
				l2_norm = torch.linalg.norm(torch.cat([p.grad.data.view(-1) for p in model_interface.e2vid_model.parameters() if p.grad is not None]))
				writer.add_scalar('stats/grad_l2_norm', l2_norm.item(), global_step)

			if all_params:
				all_params_tensor = torch.cat(all_params)
				writer.add_scalar('stats/param_avg_abs', all_params_tensor.mean().item(), global_step)
				writer.add_scalar('stats/param_max_abs', all_params_tensor.max().item(), global_step)
...
def train_stage(device, model_interface, configs, cur_epoch, optimizer, lr_scheduler, cur_stage, epochs_of_stages, writer, local_rank):
	...
	for epoch in range(rel_epoch, stage_config["max_epochs"]):
		...
		train(model_interface, train_dataloader, optimizer, device, writer, cur_epoch, local_rank, make_preview=is_val_epoch)
		lr_scheduler.step()
		if is_val_epoch:
			total_loss = validate(model_interface, val_dataloader, device, writer, cur_epoch, local_rank)
...
def main(configs):
	...
	assert task in ["e2vid", "flow", "adaptive_e2vid"]
	if task == "e2vid":
	...
	elif task == "adaptive_e2vid":
		model_interface = SparseE2VIDInterface(configs["module"], device=device, local_rank=local_rank)
