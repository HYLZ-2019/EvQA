...
def nan_hook(self, inp, output):
	...
	for i, out in enumerate(all_outputs):
		...
		if nan_mask.any():
			...
			total_cnt = out.numel()
			print(f"Found NAN in output {i} at {nan_cnt}/{total_cnt} indices: ", nan_mask.nonzero())
		if inf_mask.any():
			print("In", self.__class__.__name__)
			inf_cnt = inf_mask.sum().item()
			total_cnt = out.numel()
			print(f"Found INF in output {i} at {inf_cnt}/{total_cnt} indices: ", inf_mask.nonzero())
		if nan_mask.any() or inf_mask.any():
			traceback.print_stack()
			raise RuntimeError("NaN or Inf detected")
		#print(f"Min and max out of {self.__class__.__name__} output {i}: ", out.min().item(), out.max().item())
